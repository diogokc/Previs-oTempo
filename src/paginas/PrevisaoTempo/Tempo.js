...
export default function Tempo({route}){
  ...
  const [horarioLocal, setHorarioLocal] = useState('21')
  ...
  async function getLocation(){
    await setLocationCoords({latitude: latitude, longitude: longitude})  
  }
  ...
  async function setCurrentWeather(){
    ...
    let horario = new Date(parce.Date())
    console.log(horario)
    setHorarioLocal( ' ' + horario.getHours() +  ':' + horario.getMinutes())
    ...
    <ThemeContext.Provider value = {themeHook}>
      <View style={styles.container}>
        ...
        <TouchableOpacity style={styles.refreshButton} onPress={() => setCurrentWeather()}>
          <EvilIcons name="refresh" color={darkTheme ? 'white'  : 'black'} size={30}/>
        </TouchableOpacity>
        ...
        <Text style={styles.localizationText}>{locationName}{horarioLocal} </Text>
